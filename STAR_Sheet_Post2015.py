...
STARadmin = pd.read_csv('C:\WinPython-64bit-3.5.4.1Qt5\Glucose\STARPatients.csv')

"""Loading Patient Data and Iterates through it"""
Gluc = pd.DataFrame()
for subdir, dirs, files in os.walk('C:\WinPython-64bit-3.5.4.1Qt5\Glucose\PatientStructs2015'):
    for file in files:       
        ...
        Operative = 'True'
        Prot = datetime.datetime(int(Patient['StartDate'][0,0][0,0][0][0][0:4]), int(Patient['StartDate'][0,0][0,0][0][0][5:7]), int(Patient['StartDate'][0,0][0,0][0][0][8:10]), int(Patient['StartDate'][0,0][0,0][0][0][11:13]), int(Patient['StartDate'][0,0][0,0][0][0][14:16]), int(Patient['StartDate'][0,0][0,0][0][0][17:19]))
        Admis = datetime.datetime(int(Patient['StartDate'][0,0][0,0][0][0][0:4]), int(Patient['StartDate'][0,0][0,0][0][0][5:7]), int(Patient['StartDate'][0,0][0,0][0][0][8:10]), int(Patient['StartDate'][0,0][0,0][0][0][11:13]), int(Patient['StartDate'][0,0][0,0][0][0][14:16]), int(Patient['StartDate'][0,0][0,0][0][0][17:19]))
        if STARadmin['Prot'].str.contains(str(Prot)).any():
            print(Prot)
            continue
        PreProt = Prot-Admis
        ...
        GlucData = pd.DataFrame({'Patient': file[:-4], 'Gt': Greal[2+idx:,0], 'Gt-1': Greal[1+idx:-1,0], 'Gt-2': Greal[idx:-2,0], 't': Treal[2+idx:,0], 't-1': Treal[1+idx:-1,0], 't-2': Treal[idx:-2,0], 'ut': ures[1+idx:], 'ut-1': ures[idx:-1], 'ut-2': ures[idx-1:-2], 'Pt': Ptot[idx+1:], 'Pt-1': Ptot[idx:-1], 'Pt-2': Ptot[idx-1:-2], 'SIt': RawSI[2:], 'SIt-1': RawSI[1:-1], 'SIt-2': RawSI[0:-2], 'GF': GoalFeed[0,0], 'SIt+1': RawSIplus[2:], 'Operative': Operative, 'Gender': Gender[0], 't0': PreProt}, index = range(len(Greal)-2-idx))
