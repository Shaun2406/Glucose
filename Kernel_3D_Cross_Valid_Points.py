...
3D Method Broken Into Five Sets, Outputs PDFs For Every Test Point
"""
...
def pdf_sample(measured_pt, idx):
    ...
    for i in range(len(input_pts)):
        ...
        if xy_dist < 5*input_pts[i,3]:
            ...
            xy_comp = 1/((2*np.pi)**1.5*input_pts[i,3]**3)*np.exp(-0.5*((sample_pts[0,0]-input_pts[i,0])**2+(sample_pts[0,1]-input_pts[i,1])**2)/input_pts[i,3]**2)
            for j in range(int(np.max([z_pts - z_dist, 0])), int(np.min([z_pts + z_dist, res]))):
                density_func[j] = density_func[j]+xy_comp*np.exp(-0.5*((sample_pts[j,2]-input_pts[i,2])**2)/input_pts[i,3]**2)
    out_array = np.frombuffer(out.get_obj()).reshape((test_length, res))
    out_array[idx,:] = density_func
    
def load_data():
    #LOADING DATA
    glucData = pd.read_csv('GlucData' + str(Output) + 'H_Overall.csv')
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        np.save('..//CV Results//PDF_3D_'  + str(Output) + 'H_' + str(i+1), density_func)
        np.save('..//CV Results//TEST_3D_' + str(Output) + 'H_' + str(i+1), test_pts)