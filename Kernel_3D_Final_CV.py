...
'''def xval_gen(glucData, n):
    target = round(len(glucData)/n)
    CrossValid =  [0, 0, 0, 0, 0]
    i = 0
    temp = np.zeros([0, 4])
    for Patient, Data in glucData.groupby('Patient'):
        if len(temp) < target:
            temp = np.append(temp, Data.loc[:,['SIt', 'Gt', 'SIt+1', 'Sigma']].values, 0)
        else:
            CrossValid[i] = temp
            temp = Data.loc[:,['SIt', 'Gt', 'SIt+1', 'Sigma']].values
            i = i+1
        CrossValid[n-1] = temp
    return CrossValid'''
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        with Pool(processes=8, initializer=init_worker, initargs=(density_func_raw, means, grid_pts, trf, res, input_pts, len(test_pts), zscale)) as pool:
            pool.starmap(pdf_sample, [(test_pts[j,:], j) for j in range(1000)])
        print(time() - start)
        ...
        #plt.figure()
        #for k in range(100):
        #    plt.plot(grid_pts, density_func[k,:])